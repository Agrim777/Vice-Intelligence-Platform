...
import { VEHICLES, VEHICLE_CLASSES } from "@/data/vehicles";
...
export function Vehicles() {
  ...
  const filtered = VEHICLES
    .filter((v) => {
      ...
      const matchSearch = !search || v.name.toLowerCase().includes(search.toLowerCase()) || v.class.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Stats, prices, locations, and performance data for every top vehicle in GTA V / Online.</p>
      </div>
          ...
          {VEHICLE_CLASSES.map((c) => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-xs font-semibold border transition-colors ${
                vehicleClass === c ? "bg-primary/15 text-primary border-primary/40" : "bg-secondary text-muted-foreground border-border hover:text-foreground"
              }`}
            >
              {c}
            </button>
          ...
          <div key={i} className="border border-border bg-card rounded-md p-5 hover:border-primary/25 hover-lift transition-all">
            ...
            <div className="space-y-2 mb-4">
              <div className="grid grid-cols-[80px_1fr] items-center gap-2 text-xs text-muted-foreground">
                <span>Top Speed</span>
                <StatBar value={vehicle.topSpeed} max={140} color="bg-primary" />
              </div>
              <div className="grid grid-cols-[80px_1fr] items-center gap-2 text-xs text-muted-foreground">
            ...
            <div className="mt-1 text-xs">
              ...
              <span className="text-cyan-400 font-medium">{vehicle.bestFor}</span>
            </div>
          </div>
        ))}
      </div>
